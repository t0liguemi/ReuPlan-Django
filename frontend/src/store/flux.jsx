      ...
      actions: {
         getSession: () => {
            return fetch("api/auth", {
               ...
               headers: {
                  Authorization:
                     "Token " + localStorage.getItem("reuPlanToken"),
               },
            })
               .then((response) => {
         ...
         editEvent: (e, id) => {
            if (e.target[3].value < e.target[2].value) {
               toast.error(
                  "El final del evento debe ser posterior a su inicio"
               );
               return {};
            }
            const actions = getActions();
         ...
         uninviteUser: (id) => {
            ...
            fetch("api/invite/delete/" + id, {
               headers: {
                  Authorization:
                     "Token " + localStorage.getItem("reuPlanToken"), //ENCONTRAR EVENTO ACA
               },
            })
               .then((resp) => {
                  if (resp.status === 429) {
         ...
         createUser: (e, navigate) => {
            ...
            if (!usernamePattern.test(e.target[1].value)) {
               toast.error("El nombre de usuario solo puede contener minúsculas, mayúsculas y números.");
               return;
               ...
               toast.error("La contraseña debe tener al menos 10 caracteres y contener al menos una letra mayúscula, un símbolo y un número.");
               return;
            } else {
         ...
         checkEachInviteesResponses: () => {
            ...
            if (store.evento.final) {
               totalDays = //cantidad de dias en que puede hacerse el evento
                  (store.evento.final.getTime() -
                     store.evento.inicio.getTime()) /
                  (1000 * 3600 * 24);
            }
